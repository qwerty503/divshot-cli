...
Build.prototype.create = function (payload, done) {
  ...
  this.app.builds.create(payload, function (err, buildData) {
    ...
    if (buildData.error === 'invalid_token') return done('You need to log in before you can do this');
    if (!buildData.id) return done('You can\'t release an app that doesn\'t exist!');
